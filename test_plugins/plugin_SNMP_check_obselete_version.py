...
def run(target, pcallback):
    pcallback.log("%s serves obsolete SNMPv%s (%s); this is insecure --should upgrade to version 3" %(target.get("ip"), target.get("version"), target.get("community"))) # XXX report vuln!
